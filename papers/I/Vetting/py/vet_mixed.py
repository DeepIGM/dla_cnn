...
def test_to_tbl(test_file):
    """  Grab DLAs and SLLS from the test file
    Both are labeled as DLAs
...
def score_ml_test(dz_toler=0.015, outfile='vette_mix.json',
    ...
    for ii in range(ntest):
        ...
        dla_mts = np.where(np.abs(ml_abs['zabs'][in_sl] - mix_dlas['zabs'][ii]) < dz_toler)[0]
        nmt = len(dla_mts)
        if nmt == 0:  # No match within dz
            ...
            imin = np.argmin(np.abs(ml_abs['zabs'][in_sl] - mix_dlas['zabs'][ii]))
    ...
    for ii in range(n_slls):
        ...
        slls_mts = np.where(np.abs(ml_abs['zabs'][in_sl] - mix_slls['zabs'][ii]) < dz_toler)[0]
        nmt = len(slls_mts)
        if nmt == 0:  # No match within dz
            pass
        elif nmt == 1:  # One match
            ...
            print("Double hit in mix DLA {:d}".format(ii))
            imin = np.argmin(np.abs(ml_abs['zabs'][in_sl] - mix_slls['zabs'][ii]))
    ...
    print("There were {:d} SLLS recovered out of {:d}".format(np.sum(slls_match), n_slls))
    print("There were {:d} false positive SLLS".format(np.sum(ml_slls)-np.sum(slls_match)))
